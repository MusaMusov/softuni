function football(input) {
    ...
    let desiredPrice = Number(input[1]);

    let shortsPrice = shirtPrice * 0.75;
    let socksPrice = shortsPrice * 0.2;
    let shoesPrice = (shirtPrice + shortsPrice) * 2;

    let totalSum = (shirtPrice + shoesPrice + shortsPrice + socksPrice) * 0.85;

    if(totalSum >= desiredPrice) {
        console.log("Yes, he will earn the world-cup replica ball!");
        console.log(`His sum is ${totalSum.toFixed(2)} lv.`);
    } else {
        console.log("No, he will not earn the world-cup replica ball.");
        console.log(`He needs ${(desiredPrice - totalSum).toFixed(2)} lv. more.`);
    }
